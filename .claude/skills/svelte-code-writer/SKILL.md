...
> The same capabilities are exposed by the project's `svelte` MCP server
> (`.mcp.json`, `https://mcp.svelte.dev/mcp`) — prefer its `list-sections`,
> `get-documentation`, and `svelte-autofixer` tools when the MCP is connected.
> The `npx @sveltejs/mcp` CLI below is the fallback for when it is not.
