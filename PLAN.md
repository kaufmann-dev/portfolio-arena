...
3. **Costs: flat bps on turnover.** Every trade pays a configurable cost (default 10 bps of traded notional), across all instrument types. Frictionless tracking would systematically flatter high-turnover AI portfolios against buy-and-hold SPY.
...
- `GET /api/leaderboard` — all portfolios with metrics, benchmark rows included, plus flags (`too_early`, `stale_data`, `busted`, instrument caveat labels)
...
2. **Portfolio detail `/p/{slug}`** — base-100 NAV vs SPY chart with allocation-date markers; metrics row; current drifted holdings table (weight, side, instrument, drift since last rebalance) with long/short/futures/cash grouping; allocation timeline, each entry expandable to positions + raw response; stale-data/delisting/caveat warnings.
...
No LLM API calls, no automation of weekly runs, no response parsing, no IBKR or any broker, no options, no intraday prices, no borrow fees / cash interest / futures financing (documented simplifications), no margin-call modeling beyond the NAV ≤ 0 blow-up rule, no multi-user accounts, no email/notifications, no historical backtesting, no statistical-significance machinery beyond the age badge (revisit after ~1 year of data).
