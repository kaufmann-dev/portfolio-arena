...
- `backend/app/evaluator/` runs the integrated Codex, Muse Code, OpenCode, and Antigravity workers. Each harness has separate
  ...
  Antigravity uses native `agy --print` with a JSON schema and isolated `--gemini_dir` under
  `AGY_HOME`. Preserve its native login, allow only research tools, and validate `structured_output`;
