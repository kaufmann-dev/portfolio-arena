...
- `backend/app/evaluator/` runs the integrated Codex and Muse Code workers. Each harness has separate
  authentication and health; queue concurrency is shared. Muse imports its authenticated model catalog
  through a private internal route. Workers receive only deployment credentials
