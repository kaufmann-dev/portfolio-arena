...
<article class="about">
  ...
  <p>
    A long-term experiment: <strong>can LLMs pick portfolios that beat SPY?</strong> On a recurring cadence,
    the operator prompts AI agents (Claude, Codex, Gemini, …) with portfolio-management prompts and enters
    each agent's proposed allocation here by hand. The app simulates it as a paper portfolio from Yahoo
    Finance data and tracks it against SPY on the
    <a href="/" onclick={(e) => link(e, "/")}>public leaderboard</a>. The app never calls an LLM — it is an
    arena for honest, deterministic measurement.
  </p>

  <h2>Integrity rules</h2>
  <ul>
    <li>
      <strong>No backdating, no lookahead.</strong> An allocation entered at time T takes effect at the first market
      close strictly after T. Entered Saturday → effective Monday's close. Prices before entry can never be claimed.
    </li>
    <li>
      <strong>Positions lock at the effective close.</strong> Until then there is a typo-correction window; afterwards
      the positions backing the track record are frozen forever. Only metadata (prompt reference, notes, raw response)
      stays editable.
    </li>
    <li>
  ...
  <ul>
    ...
    <li>
      Total-return basis: Yahoo <em>adjusted closes</em> (dividends included) for positions and for SPY. Base currency
      is USD.
    </li>
    <li>
