...
export function metaBatchStatusCopy(status: MetaBatchStatus): string {
  if (status === "ready") {
    return "Each meta run uses frozen sources matching its harness, managed/rebuilt mode, and long/short direction.";
  }
  ...
  return "Meta evaluations begin after every due normal-portfolio run for the same harness reaches a terminal state.";
}
